...
rt_input = 
  df %>%
  mutate(genre = str_c(genre_tag, " ", genre)) %>%
  ...
  details = function(index) {
    ...
    detail = 
      div(
        ...
        # Review
        div(
          div(class = "film-detail-name", glue("Our Review [{str_to_upper(film$reviewer)}]")), 
          # markdown::markdownToHTML(
          #   text = if(is.na(film$review_text)) "<div></div>" else film$review_text,
          #   fragment.only = TRUE
          # ) %>%
          
          if (is.na(film$review_text)) {
            p()
          } else {
            p(film$review_text)
