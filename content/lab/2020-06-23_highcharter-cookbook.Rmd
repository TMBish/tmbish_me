...
tags: ["R", "Highcharter"]
highlight: "kate"
description: "A cookbook for using the highcharter R package"
img: "highcharter-cookbook"
output: 
    blogdown::html_page:
        toc: true
---
...
So why would you use `highcharter` at all then? 
The answer is that it has a huge number of functional and stylistic advantages over it's static counterparts. 
Data analytics & data science outputs are becoming almost exclusively viewable through web browsers so it's extremely valuable to be able to create charts that are built to be viewed in a web browser. `ggvis` was supposed to be this solution and a successor to `ggplot` but it is half built and it's development has been delayed indefinitely.
`highcharter` is somewhat limited compared with `ggplot`, however, that's not an indictment because `ggplot` is probably the most mature charting library in data analytics history.
The most important point to emphasise though is impact. The impact of suprising your stakeholders with interactive, clean, and aesthetically appealing charts is hugely underated and you're exponentionally more likely to get buy-in on your technical work if you can pair good solid technical analysis with really shiny packaging.
...
![](img/lab/hccook/hc_yAxis_help.PNG)
...
![](img/lab/hccook/hc_yAxis_examples.PNG)
...
The answer is to refer to the API documentation. Let's check the actual `highcharts`  API documentation for a `highcharts`  y Axis. You can find it yourself [here](https://api.highcharts.com/highcharts/yAxis) but a screen shot of the page looks like this:

![](img/lab/hccook/yAxis_api_doc.PNG)
...
![](img/lab/hccook/yaxis_dash_style.PNG)
...
![](img/lab/hccook/yAxis_title_api_doc.PNG)
...
![](img/lab/hccook/series_column.PNG)
...
![](img/lab/hccook/hc_columns_data_1.PNG)
...
![](img/lab/hccook/hc_columns_data_2.PNG)
...
![](img/lab/hccook/hc_columns_data_3.PNG)
...
![](img/lab/hccook/highcharts_margins.png)
...
* To show you how you can work backwards directly from charts made purely for the highcharts javascript library I went to the [demos page](https://www.highcharts.com/demo) and tried to replicate [this packed bubble chart](https://www.highcharts.com/demo/packed-bubble-split)
* Once you understand the *Understanding The API* section of this document working backwards from these demo charts becomes reasonably easy
* This chart type should highlight the powerful visual effects of the package and those that you can leverage to make an impact when you show off your outputs to your various stakeholders
* I've also added a tooltip chart to make it a little cooler and complex
...
* Full diclosure this isn't my idea at all, I just think it's an awesome graph. Josh Kunst's version of this chart is located on one of his blogposts here: http://jkunst.com/blog/posts/2019-02-04-using-tooltips-in-unexpected-ways/
* For my version I'd like to plot scoring efficiency for each team as measured by True Shooting %
* I'll use a donut arrangement not to present any data dimension but to just layout the all teams in a ring
* The team based visual will be presented on hover over in the form of a fixed tooltip located in the center of the donut
        ...
        yAxis = list(
          ...
          plotLines = list(
            list(
              value = leagueTS,
              zIndex = -100,
              color = "rgba(212, 217, 214, 0.4)",
              label = list(text = "League Average", align = "right", style = list(fontWeight = "bold", color = "rgba(212, 217, 214, 0.4)")
              )
            )
          )
        ),
...
# An example of one of the point configurations
playerSeries[[1]]$sequence[[1]]
```
